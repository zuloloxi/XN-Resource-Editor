...
begin
  ...
  Width := 300;
  Height := 200;
  for i := Low (FHexData) to High (fHexData) do
    for j := 0 to 2 do
      FHexData [i][j] := #9;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if NonPrintableChar <> #0 then
          begin
            SetString (st, LineChars(I+FTopLine), ByteCnt);


            for j := 1 to Length (st) do
              if (st [j] >= #$80) or (st [j] < ' ') then
                st [j] := NonPrintableChar;

            ExtTextOutA(Canvas.Handle, R.Left, R.Top, ETO_OPAQUE or ETO_CLIPPED, @R, PAnsiChar (st), Length (st), nil)
          end
          else
...
begin
  Result := StrFmt(FLineAddr, '%p:', [Pointer(PByte(AddressOffset)+Index*FBytesPerLine)]);
end;
...
begin
  ...
  begin
    ...
    if changes then
    begin
      PAnsiChar (Address) [Offset] := AnsiChar (ch);
      SetChanged;
